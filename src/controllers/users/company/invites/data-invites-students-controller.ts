...
export async function controllerInviteNotAcceptedStudents(request: FastifyRequest, reply:FastifyReply){
    try{
        ...
        const existsInvitesCompany = await prisma.inviteCompany.findMany({where:{companyId:companyId, accepted:false}});

        if(!existsInvitesCompany) return reply.status(409).send({message:"Não há registros de convites não aceitos..."});
