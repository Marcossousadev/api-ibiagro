...
export async function sendEmailInviteCodeProfessional(to: string, fullName:string, nameProfessional: string, code: string, expiresAt: Date, date:Date | string, message: string) {
try{
  const {data, error}  = await resend.emails.send({
    from: 'Fit Pro <onboarding@resend.dev>',
    to,
    subject:'Seu código de convite do Professional',
    html:`
      <div style="font-family: sans-serif; padding: 10px;">
      <h1>Sou o profissional, ${nameProfessional}</h1>
      <h1>Olá, ${fullName}</h1>
